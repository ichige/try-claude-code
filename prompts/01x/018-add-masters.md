...
先ほどの手ごたえから言えば、短い指示でも全部作ってくれそうである。

```markdown
packages/shared/src/types/consignees.ts に ConsigneesItem を作成しました。
apps/admin/src/router/routes.ts の masters-container で CRUD 出来るように、
stores の整備や configs への設定追加、i18n などの調整をお願いします。
基本的には ConsignorsItem と同じ取り扱いになります。
```

2分弱ほどで完成。やはりある程度構成が出来てしまうと、速度も精度も高い。

よく見ると、i18n対応漏れがあるので、そこを修正しておく。

```markdown
configs/dialog-form 配下の設定だけど、「基本情報」や「所在地」などがまだ i18n 対応してないようである。  
icon も configs/icon.ts に統合されていない。
取り扱うマスタデータの種類によってiconを変更する必要もないので、対応をお願いします。
---
項目は単なるラベルなので、labels.xxx として設定してください。
---
そうではなく、section というカテゴリが不要なのです。
---
icon も統合してください。あくまで label に対して一つで問題ないです。
```

現状維持を心がけるのか？やけに回りくどい修正をするケースが目立つことがある。  
もっとシンプルに修正するように指示しないと、無駄の多いコードを大量に生産する恐れはあるかもしれない。
