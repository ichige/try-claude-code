...
```markdown
apps/admin/src/components/masters/ContainerTable.vue に簡易フィルタを追加できるか？
---
検索 input は、titleKey を表示している隣に配置してくれ。spacer 使って間隔をあけておくほうが良いだろう。
`common.search` は `labels.search` へ変更してほしい。
configs/icon.ts に icon 設定を追加しておいいて。
```
