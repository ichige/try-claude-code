...
VeeVakidate は Typescript との相性や、ref の管理が煩雑になるなど問題がある。  
一方として、zod を利用した場合、スキーマバリデーションとしては便利ではあるが、入力ごとのリアルタイム検証には向いていない。  
まず設計として、この状況をいかに打破するか？が問題になってくる。

```markdown
`DialogForm.vue` で zod でスキーマを定義してほしい。
- companyName と paymentRate は必須。
- paymentRate は 数値であること。(要キャスト)
- それ以外は文字列であること。(デフォで空文字)
エラーメッセージはzodに設定できないのか？ 
rules は _ZodType 型を引数とることで、共通関数化できるのでは？
```
...
このフォームの構成を設定化して、汎用フォームとして振る舞う設計に変更していく。  
これは非常に面倒だけど、有意義な事である。

```markdown
まず initDialogForm の引数を container 名と operation 名(create/update) とする。
- 設定はどこまで使いまわすが不明なので、`configs/dialog-form` 配下あたりに2種配置すると良いだろう。
- 続いて分かりやすいところで、DialogForm.vue の zod スキーマ、reactive(form変数)など、直接的に Consignors に関連する設定を `configs/dialog-form` に切り出す。
- initDialogForm でコンテナに関連する設定を動的に読み込み、useDialogFormXXX から export してみる。
- DialogForm 側の変数は汎用的な名称に変更して、とりあえずテンプレートへあててみる。
```

エラーが出るので修正させる。

```markdown
[ESLint] All imports in the declaration are only used as types. Use `import type`. (@typescript-eslint/consistent-type-imports)
/home/magro/projects/try-claude-code/apps/admin/src/components/dialogs/DialogForm.vue:148:1
これを調べて。
```

エラーが解消したところで、今度はフォームの構成を大改造する。
...
---
こんな構造で入力カテゴリのヘッダがあらわせるかな。
それ以外はデザインということで、そのまま DialogForm に残しておけそうだ。
```
...
---
入力はこれくらいシンプルで構成できるのでは？
```

```markdown
となると、reactive の生成も composable で完結して、onSubmit 関数を export すれば、ストアの操作ロジックまで封じ込められるかな？
この理論で書きかえらそうか？
do it
```
...
```markdown
ところで、dialog-form.ts で sections(Computed) を使って構成を変更しているのは、あえて型を意識しての事なのかな？
...
```markdown
components/dialogs/DialogForm.vue で「会社情報登録」と固定で表示しているけど、
configs/dialog-form/*.ts から設定をロードすべきかと思う。
そのメタデータ的なデータを export しつつ、○○登録と○○更新と出し分けて欲しいのだが、出来そう？
```

ここまで来ると、ほぼ完璧にこなしてくれる。  
コードの組み立てパターンは大事だ。
