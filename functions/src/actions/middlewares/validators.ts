 ...
 * @returns ミドルウェア関数
 */
export const validateParams2 = <T extends Record<string, string>>(schema: ZodType<T>) =>
  async (passable: Passable, next: NextFunction<Passable, Passable>): Promise<Passable> => {
    ...
    return next(passable.mergeParams(result.data))
  }
 ...
 * @returns ミドルウェア関数
 */
export const validateBody2 = <T extends Record<string, unknown>>(schema: ZodType<T>) =>
  async (passable: Passable, next: NextFunction<Passable, Passable>): Promise<Passable> => {
    const result = schema.safeParse(await passable.request.json())
    if (!result.success) throw new ValidationError(result.error)
    return next(passable.mergeBody(result.data))
  }
